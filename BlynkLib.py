...
print("""
  ...
  / _  / / // / _ \\/  '_/
 /_//\\, /////\\_\\
        /_/ for Python v""" + _version_ + " (" + sys.platform + ")\n")
...
class BlynkProtocol(EventEmitter):
    def _init_(self, auth, tmpl_id=None, fw_ver=None, heartbeat=50, buffin=1024, log=None):
        EventEmitter._init_(self)
            ...
            if cmd == MSG_RSP:
                ...
                if self.state == CONNECTING and i == 1:
                    if dlen == STA_SUCCESS:
                        ...
                        info = ['ver', _version_, 'h-beat', self.heartbeat//1000, 'buff-in', self.buffin, 'dev', sys.platform+'-py']
                        if self.tmpl_id:
